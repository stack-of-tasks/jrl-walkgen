...
void OrientationsPreview::preview_orientations(double Time, const reference_t &Ref, double StepDuration,
  ...
  while (!TrunkVelOK) {
    ...
    if (CurrentSupport.Phase != DS) {
      ...
      while (!TrunkAngleOK) {
        ...
        if (fabs(TrunkStateT_.yaw[1] - TrunkState_.yaw[1]) > EPS_) {
          TrunkStateYaw_->SetParameters(T_, TrunkState_.yaw[0], TrunkState_.yaw[1],
                                        /*initAcc*/ 0.0,
        ...
        TrunkAngleOK = verify_angle_hip_joint(CurrentSupport, PreviewedTrunkAngleEnd, TrunkState_, TrunkStateT_,
    ...
    // ------------------------
    for (StepNumber = (unsigned)FirstFootPreviewed; StepNumber <= (unsigned)((int)ceil((N_ + 1) * T_ / StepDuration));
         StepNumber++) {
      ...
      verify_velocity_hip_joint(Time, PreviewedSupportFoot, PreviewedSupportAngle, StepNumber, CurrentSupport,
                                CurrentRightFootAngle, CurrentLeftFootAngle, CurrentLeftFootVelocity,
      ...
      if (!TrunkAngleOK) {
        ...
        if (ItBeforeLanding <= ItBeforeLandingThresh && ItBeforeLanding > 0 &&
            ...
            NbStepsPreviewed > 0 && StepNumber == (unsigned)FirstFootPreviewed) {
...
bool OrientationsPreview::verify_angle_hip_joint(const support_state_t &CurrentSupport, double PreviewedTrunkAngleEnd,
  ...
  if (fabs(PreviewedTrunkAngleEnd - CurrentSupportFootAngle) > fabs(JointLimit)) {
    TrunkStateT_.yaw[1] =
        (CurrentSupportFootAngle + 0.9 * JointLimit - TrunkState_.yaw[0] - TrunkState_.yaw[1] * T_ / 2.0) /
...
void OrientationsPreview::verify_velocity_hip_joint(double Time, double PreviewedSupportFoot,
  ...
  if (StepNumber > 0 && CurrentSupport.Phase == SS) {
    ...
    if (df(a, b, c, d, temp) > uvLimitFoot_) {
      a = 0;
      c = -1.0 / (2.0 * T) *
          (2.0 * b - 2.0 * uvLimitFoot_ + 2.0 * sqrt(uvLimitFoot_ * uvLimitFoot_ - b * uvLimitFoot_));
      d = (-2.0 * c - b / T) / (3.0 * T);
...
void OrientationsPreview::interpolate_trunk_orientation(double Time, int CurrentIndex, double NewSamplingPeriod,
  ...
  if (CurrentSupport.Phase == SS &&
      Time + 1.5 * T_ < CurrentSupport.TimeLimit)  // CurrentSupport.Phase == SS && Time+3.0/2.0*T_ <
...
void OrientationsPreview::one_iteration(double Time, const deque<support_state_t> &PrwSupportStates_deq) {
  support_state_t CurrentSupport = PrwSupportStates_deq.front();
