...
void NMPCgenerator::initNMPCgenerator(bool useLineSearch, support_state_t &currentSupport, COMState &lStartingCOMState,
...
void NMPCgenerator::updateConstraint() {
  ...
  for (unsigned i = 0; i < nc_cop_; ++i) {
    ...
    for (unsigned j = 0; j < nf_; ++j) qp_J_(i + index, j + N2nf2) = Acop_theta_(i, j);
  }
  index = (unsigned int)nc_cop_;
  for (unsigned i = 0; i < nc_foot_; ++i) {
    for (unsigned j = 0; j < N2nf2; ++j) qp_J_(i + index, j) = Afoot_xy_full_(i, j);
    for (unsigned j = 0; j < nf_; ++j) qp_J_(i + index, j + N2nf2) = Afoot_theta_full_(i, j);
  }
  index += nc_foot_;
  ...
  for (unsigned obs = 0; obs < obstacles_.size(); ++obs) {
    for (unsigned n = 0; n < nf_; ++n) {
      ...
      for (unsigned j = 0; j < N2nf2; ++j) qp_J_((obs + 1) * n, j) = qp_J_obs_i_(j);
    }
  }
...
void NMPCgenerator::evalConstraint(Eigen::VectorXd &U) {
  ...
  for (unsigned obs = 0; obs < obstacles_.size(); ++obs) {
    for (unsigned n = 0; n < nf_; ++n) {
      ...
      double deltaObs = 0;
      for (unsigned i = 0; i < HobsUxy.size(); ++i) deltaObs += Uxy_(i) * (HobsUxy(i) + Aobs_[obs][n](i));
...
void NMPCgenerator::updateInitialCondition(double time, FootAbsolutePosition &currentLeftFootAbsolutePosition,
  ...
  updateCurrentSupport(time_, currentLeftFootAbsolutePosition, currentRightFootAbsolutePosition);

  if (currentSupport_.StateChanged && desiredNextSupportFootRelativePosition.size() != 0) {
    desiredNextSupportFootRelativePosition.pop_front();
  }

#ifdef DEBUG_COUT
  cout << time_ << "  " << currentSupport_.StartTime << "  " << currentSupport_.TimeLimit << "  "
       << currentSupport_.NbStepsLeft << "  " << endl;
#endif
  if (currentSupport_.TimeLimit > 1e+8) {
    Tfirst_ = T_;
  } else if (currentSupport_.StartTime == 0.0) {
    Tfirst_ =
        (time_ + T_ - currentSupport_.TimeLimit) - ((double)(int)((time_ - currentSupport_.TimeLimit) / T_) * T_);
#ifdef DEBUG_COUT
    cout << Tfirst_ << " ";
#endif
    Tfirst_ = T_ - Tfirst_;
  } else {
    Tfirst_ = (time_ - currentSupport_.StartTime) - ((double)(int)((time_ - currentSupport_.StartTime) / T_) * T_);
    Tfirst_ = T_ - Tfirst_;
  }
  ...
  updateSupportdeque(time_, currentLeftFootAbsolutePosition, currentRightFootAbsolutePosition);
  ...
  for (unsigned i = 0; i < N_; ++i) {
    if (SupportStates_deq_[i + 1].Phase == DS) {
      if (SupportStates_deq_[i + 1].Foot == LEFT) {
        v_kp1f_x_(i) =
            v_kp1_(i) * (SupportStates_deq_[i + 1].X + sin(SupportStates_deq_[i + 1].Yaw) * FeetDistance_ * 0.5);
        v_kp1f_y_(i) =
            v_kp1_(i) * (SupportStates_deq_[i + 1].Y - cos(SupportStates_deq_[i + 1].Yaw) * FeetDistance_ * 0.5);
      } else {
        v_kp1f_x_(i) =
            v_kp1_(i) * (SupportStates_deq_[i + 1].X - sin(SupportStates_deq_[i + 1].Yaw) * FeetDistance_ * 0.5);
        v_kp1f_y_(i) =
...
void NMPCgenerator::solve() {
  ...
  while (iter < maxSolverIteration_ && oneMoreStep_ == true) {
    ...
    normDeltaU = 0.0;
    for (unsigned i = 0; i < nv_; ++i) normDeltaU += sqrt(deltaU_[i] * deltaU_[i]);
...
void NMPCgenerator::solve_qp() {
  // primal SQP solution
  QP_->solve(QuadProg_H_, QuadProg_g_, QuadProg_J_eq_, QuadProg_bJ_eq_, QuadProg_J_ineq_, QuadProg_lbJ_ineq_, false);
  //  if(QP_->fail()==0)
...
void NMPCgenerator::postprocess_solution() {
  ...
  */
  ...
  cout << "delta_F_Y = [";
  for (unsigned i = 2 * N_ + nf_; i < 2 * N_ + 2 * nf_; ++i) cout << deltaU_thresh_[i] << " , ";
  cout << "]" << endl;

  cout << "delta_F_theta = [";
  for (unsigned i = 2 * N_ + 2 * nf_; i < 2 * N_ + 3 * nf_; ++i) cout << deltaU_thresh_[i] << " , ";
  cout << "]" << endl;
#endif
...
void NMPCgenerator::getSolution(std::vector<double> &JerkX, std::vector<double> &JerkY, std::vector<double> &FootStepX,
...
#ifdef DEBUG_COUT
  cout << currentSupport_.NbStepsLeft << " " << SupportStates_deq_.back().StepNumber << endl;
#endif  // DEBUG_COUT
...
void NMPCgenerator::updateCurrentSupport(double time, FootAbsolutePosition &FinalLeftFoot,
                                         FootAbsolutePosition &FinalRightFoot) {
#ifdef DEBUG_COUT
  cout << "previous support : \n"
       << currentSupport_.Phase << " " << currentSupport_.Foot << " " << currentSupport_.StepNumber << " "
       << currentSupport_.StateChanged << " " << currentSupport_.X << " " << currentSupport_.Y << " "
       << currentSupport_.Yaw << " " << currentSupport_.NbStepsLeft << " " << endl;
#endif
...
void NMPCgenerator::updateSupportdeque(double time, FootAbsolutePosition &FinalLeftFoot,
                                       FootAbsolutePosition &FinalRightFoot) {
  ...
  for (unsigned i = 0; i < SupportStates_deq_.size(); ++i) {
    cout << SupportStates_deq_[i].Phase << " " << SupportStates_deq_[i].Foot << " " << SupportStates_deq_[i].StepNumber
         << " " << SupportStates_deq_[i].StateChanged << " " << SupportStates_deq_[i].X << " "
         << SupportStates_deq_[i].Y << " " << SupportStates_deq_[i].Yaw << " " << SupportStates_deq_[i].NbStepsLeft
         << " " << endl;
  }
#endif
}
  ...
  for (unsigned i = 0; i < N_; ++i) {
    ...
    for (unsigned j = 0; j <= i; ++j) {
      ...
      if (j == 0) {
        ...
        Pau_(i, j) = T1;
        Pzu_(i, j) =
            (T1 * T1 * T1 + 3 * i_j * T_ * T1 * T1 + 3 * i_j * i_j * T_ * T_ * T1) / 6.0 - T1 * c_k_z_ / GRAVITY;
      } else {
        Ppu_(i, j) = (3.0 * i_j * i_j + 3.0 * i_j + 1) * T_ * T_ * T_ / 6.0;
        Pvu_(i, j) = (2.0 * i_j + 1) * T_ * T_ * 0.5;
        Pau_(i, j) = T_;
        Pzu_(i, j) = (3.0 * i_j * i_j + 3.0 * i_j + 1) * T_ * T_ * T_ / 6.0 - T_ * c_k_z_ / GRAVITY;
      }
    }
  }
#ifdef DEBUG
...
void NMPCgenerator::updateCoMCoPIntegrationMatrix() {
  ...
  for (unsigned i = 0; i < N_; ++i) {
    ...
    Pau_(i, 0) = T1;
    Pzu_(i, 0) = (T1 * T1 * T1 + 3 * i * T_ * T1 * T1 + 3 * i * i * T_ * T_ * T1) / 6.0 - T1 * c_k_z_ / GRAVITY;
  }
  ...
  for (unsigned n = ignoreFirstStep; n < nf_; ++n) {
    ...
    if (n != 0) {
      ...
      for (unsigned i = 0; i < n_vertices_; ++i)
        for (unsigned j = 0; j < nf_; ++j)
          Afoot_theta_full_((n - ignoreFirstStep) * n_vertices_ + i, j) = Afoot_theta_[n](i, j);
    }
  ...
  for (unsigned n = ignoreFirstStep; n < nf_; ++n) {
    ...
    for (unsigned i = 0; i < n_vertices_; ++i)
      for (unsigned j = 0; j < 2 * (N_ + nf_); ++j)
        Afoot_xy_full_((n - ignoreFirstStep) * n_vertices_ + i, j) = Afoot_xy_[n](i, j);
    ...
    if (n == 0) {
      for (unsigned i = 0; i < n_vertices_; ++i) {
        UBfoot_[0](i) += A0f_xy_[0](i, 0) * currentSupport_.X + A0f_xy_[0](i, 1) * currentSupport_.Y;
      }
    }
    for (unsigned i = 0; i < n_vertices_; ++i) UBfoot_full_((n - ignoreFirstStep) * n_vertices_ + i) = UBfoot_[n](i);
  }
  return;
}
...
void NMPCgenerator::updateObstacleConstraint() {
  ...
  for (unsigned obs = 0; obs < obstacles_.size(); ++obs) {
    for (unsigned i = 0; i < nc; ++i) {
      ...
      UBobs_[obs](i) = -(-obstacles_[obs].x_0 * obstacles_[obs].x_0 - obstacles_[obs].y_0 * obstacles_[obs].y_0 +
                         (obstacles_[obs].r + obstacles_[obs].margin) * (obstacles_[obs].r + obstacles_[obs].margin));
    }
#ifdef DEBUG_COUT
...
void NMPCgenerator::updateCostFunction() {
  ...
  Q_x_XX_ = alpha_x_ * Pvu_.transpose() * Pvu_ + beta_ * Pzu_.transpose() * Pzu_ + minjerk_ * I_NN_;
  ...
  Q_x_FF_ = beta_ * V_kp1_.transpose() * V_kp1_ + delta_ * I_FF_ + kappa_ * diffMat_.transpose() * diffMat_;

  // Q_yXX = (  0.5 * a * Pvu^T   * Pvu + b * Pzu^T * Pzu + c * I )
  Q_y_XX_ = alpha_y_ * Pvu_.transpose() * Pvu_ + beta_ * Pzu_.transpose() * Pzu_ + minjerk_ * I_NN_;
  ...
  for (unsigned i = 0; i < nf_; ++i)
    for (unsigned j = 0; j < nf_; ++j) qp_H_(i + N2nf2, j + N2nf2) = Q_theta_(i, j);
  ...
  p_xy_Fx_ = -beta_ * V_kp1_.transpose() * (Pzsc_x_ - v_kp1f_x_) - delta_ * I_FF_ * F_kp1_x_;
  ...
  p_xy_Fy_ = -beta_ * V_kp1_.transpose() * (Pzsc_y_ - v_kp1f_y_) - delta_ * I_FF_ * F_kp1_y_;
  ...
  for (unsigned i = 0; i < nf_; ++i)
    p_(index + i) =
        -alpha_theta_ * (currentSupport_.Yaw + (i + 1) * T_step_ * vel_ref_.Global.Yaw) - delta_ * F_kp1_theta_(i);
...
#endif
  vel_ref_.Global.X_vec = Eigen::VectorXd::Constant(vel_ref_.Global.X_vec.size(), vel_ref_.Global.X);
  vel_ref_.Global.Y_vec = Eigen::VectorXd::Constant(vel_ref_.Global.Y_vec.size(), vel_ref_.Global.Y);
#ifdef DEBUG
  DumpVector("RefVectorX", vel_ref_.Global.X_vec);
  DumpVector("RefVectorY", vel_ref_.Global.Y_vec);
#endif
