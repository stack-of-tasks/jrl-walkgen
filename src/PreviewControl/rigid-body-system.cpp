...
RigidBodySystem::RigidBodySystem(SimplePluginManager *SPM, PinocchioRobot *aPR, SupportFSM *FSM)
    : mass_(0), CoMHeight_(0), T_(0), Tr_(0), Ta_(0), N_(0), multiBody_(false), OFTG_(0), FSM_(0) {
  PR_ = aPR;
  FSM_ = FSM;
...
int RigidBodySystem::initialize_trajectories() {
  ...
  for (unsigned int i = 0; i < NbInstantsSS; i++) {
    if (i * T_ > SSPeriod / 2.0) {
      ...
      OFTG_->SetParameters(FootTrajectoryGenerationStandard::Z_AXIS, SSPeriod / 2.0, 0.0, OFTG_->GetStepHeight(), 0.0,
                           0.0);
    }
...
int RigidBodySystem::precompute_trajectories(const deque<support_state_t> &SupportStates_deq) {
  ...
  for (unsigned int i = 0; i < N_; i++) {
    ...
    unsigned int endLoop = std::min<unsigned>(SS_it->NbInstants, (unsigned int)FlyingFootTrajectory_deq_.size());
...
int RigidBodySystem::compute_dyn_cop(unsigned nbSteps) {
  ...
  double GRF = 0.0;
  for (unsigned int i = 0; i < N_; i++) {
    GRF = CoM_.Mass() * (CoMTraj_it->Z[2] + GRAVITY) + LeftFoot_.Mass() * (LFTraj_it->Z[2] + GRAVITY) +
    ...
    if (multiBody_) {
      LeftFoot_.Dynamics(COP_POSITION).S.row(i) +=
          LeftFoot_.Dynamics(POSITION).S.row(i) * LeftFoot_.Mass() * (LFTraj_it->Z[2] + GRAVITY) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).U.row(i) +=
          LeftFoot_.Dynamics(POSITION).U.row(i) * LeftFoot_.Mass() * (LFTraj_it->Z[2] + GRAVITY) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).UT.col(i) +=
          LeftFoot_.Dynamics(POSITION).U.row(i) * LeftFoot_.Mass() * (LFTraj_it->Z[2] + GRAVITY) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).S.row(i) -=
          LeftFoot_.Dynamics(ACCELERATION).S.row(i) * LeftFoot_.Mass() * (LFTraj_it->Z[0]) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).U.row(i) -=
          LeftFoot_.Dynamics(ACCELERATION).U.row(i) * LeftFoot_.Mass() * (LFTraj_it->Z[0]) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).UT.col(i) -=
          LeftFoot_.Dynamics(ACCELERATION).U.row(i) * LeftFoot_.Mass() * (LFTraj_it->Z[0]) / GRF;

      RightFoot_.Dynamics(COP_POSITION).S.row(i) +=
          RightFoot_.Dynamics(POSITION).S.row(i) * RightFoot_.Mass() * (RFTraj_it->Z[2] + GRAVITY) / GRF;
      RightFoot_.Dynamics(COP_POSITION).U.row(i) +=
          RightFoot_.Dynamics(POSITION).U.row(i) * RightFoot_.Mass() * (RFTraj_it->Z[2] + GRAVITY) / GRF;
      RightFoot_.Dynamics(COP_POSITION).UT.col(i) +=
          RightFoot_.Dynamics(POSITION).U.row(i) * RightFoot_.Mass() * (RFTraj_it->Z[2] + GRAVITY) / GRF;
      RightFoot_.Dynamics(COP_POSITION).S.row(i) -=
          RightFoot_.Dynamics(ACCELERATION).S.row(i) * RightFoot_.Mass() * (RFTraj_it->Z[0]) / GRF;
      RightFoot_.Dynamics(COP_POSITION).U.row(i) -=
          RightFoot_.Dynamics(ACCELERATION).U.row(i) * RightFoot_.Mass() * (RFTraj_it->Z[0]) / GRF;
      RightFoot_.Dynamics(COP_POSITION).UT.col(i) -=
          RightFoot_.Dynamics(ACCELERATION).U.row(i) * RightFoot_.Mass() * (RFTraj_it->Z[0]) / GRF;
    }
        ...
        for (unsigned int j = 0; j < N_; j++)
          if (j <= i)
            Dynamics.U(i, j) = Dynamics.UT(j, i) =
                (1 + 3 * (i - j) + 3 * (i - j) * (i - j)) * T_ * T_ * T_ / 6.0 - T_ * CoMHeight_ / 9.81;
          else
            Dynamics.U(i, j) = Dynamics.UT(j, i) = 0.0;
      }
      break;
...
int RigidBodySystem::compute_foot_zero_dynamics(const std::deque<support_state_t> &SupportStates_deq,
    ...
    if (SS_it->Phase == DS) {
      ...
      if (SS_it->StepNumber == 0) {
        if (FFDynamics->Type == POSITION) {
          ...
          SFDynamics->S(i, 1) = 0.0;
          SFDynamics->S(i, 2) = 0.0;
          FFDynamics->U(i, SS_it->StepNumber) = FFDynamics->UT(SS_it->StepNumber, i) = 0.0;
...
int RigidBodySystem::compute_foot_pol_dynamics(const std::deque<support_state_t> &SupportStates_deq,
                                               linear_dynamics_t &LeftFootDynamics,
                                               linear_dynamics_t &RightFootDynamics) {
  // Resize the matrices:
  // --------------------
    ...
    if (SS_it->Phase == DS) {
      ...
      if (SS_it->StepNumber == 0 && SS_it->StepNumber < nbSteps) {
        ...
        if (((SS_it->NbInstants) * T_ > FSM_->StepPeriod() - T_) && (SS_it->StepNumber != 0)) {
          ...
          FFDynamics->S(i, 2) = FFDynamics->S(i - 1, 2);
          SFDynamics->S(i, 2) = SFDynamics->S(i - 1, 2);
          FFDynamics->U(i, SS_it->StepNumber) = FFDynamics->UT(SS_it->StepNumber, i) =
              FFDynamics->U(i - 1, SS_it->StepNumber);
          SFDynamics->U(i, SS_it->StepNumber) = SFDynamics->UT(SS_it->StepNumber, i) =
              SFDynamics->U(i - 1, SS_it->StepNumber);
        }
      } else if (SS_it->StepNumber == 1 && SS_it->StepNumber < nbSteps) {
        if (FFDynamics->Type == POSITION) {
          FFDynamics->S(i, 0) = Spbar[0];
        ...
        if ((SS_it->NbInstants) * T_ > FSM_->StepPeriod() - T_) {
          ...
          FFDynamics->U(i, SS_it->StepNumber) = FFDynamics->UT(SS_it->StepNumber, i) =
              FFDynamics->U(i - 1, SS_it->StepNumber);
          SFDynamics->U(i, SS_it->StepNumber - 1) = SFDynamics->UT(SS_it->StepNumber - 1, i) =
...
int RigidBodySystem::generate_trajectories(double Time, const solution_t &Solution,
  ...
  OFTG_->interpolate_feet_positions(Time, PrwSupportStates_deq, Solution, PreviewedSupportAngles_deq, LeftFootTraj_deq,
                                    RightFootTraj_deq);
...
int RigidBodySystem::compute_sbar(double *Spbar, double *Sabar, double T, double Td) {
  double Td2 = Td * Td;
  double Td3 = Td * Td * Td;
...
int RigidBodySystem::compute_ubar(double *Upbar, double *Uabar, double T, double Td) {
  double Td3 = Td * Td * Td;
  double Td4 = Td * Td * Td * Td;
  double Td5 = Td * Td * Td * Td * Td;
