...
PatternGeneratorInterfacePrivate::PatternGeneratorInterfacePrivate(PinocchioRobot *aPinocchioRobotRobot)
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++)
      if (i == j)
        m_MotionAbsPos(i, j) = m_MotionAbsOrientation(i, j) = m_WaistAbsPos(i, j) = m_WaistRelativePos(i, j) = 1.0;
      else
        m_MotionAbsPos(i, j) = m_MotionAbsOrientation(i, j) = m_WaistAbsPos(i, j) = m_WaistRelativePos(i, j) = 0.0;
  }
...
void PatternGeneratorInterfacePrivate::AllowFPE() { feenableexcept(FE_INVALID | FE_DIVBYZERO | FE_OVERFLOW); }
...
void PatternGeneratorInterfacePrivate::ObjectsInstanciation() {
  ...
  m_DoubleStagePCStrategy->SetBufferPositions(&m_ZMPPositions, &m_COMBuffer, &m_LeftFootPositions,
                                              &m_RightFootPositions);

  m_CoMAndFootOnlyStrategy = new CoMAndFootOnlyStrategy(this);
  m_CoMAndFootOnlyStrategy->SetBufferPositions(&m_ZMPPositions, &m_COMBuffer, &m_LeftFootPositions,
                                               &m_RightFootPositions);
...
void PatternGeneratorInterfacePrivate::InterObjectRelationInitialization() {
  ...
  m_DoubleStagePCStrategy->InitInterObjects(m_PinocchioRobot, m_ComAndFootRealization[0], m_StepStackHandler);

  m_CoMAndFootOnlyStrategy->InitInterObjects(m_PinocchioRobot, m_ComAndFootRealization, m_StepStackHandler);
...
void PatternGeneratorInterfacePrivate::initOnlineHerdt() {
  ...
  EvaluateStartingState(lStartingCOMState, lStartingZMPPosition, lStartingWaistPose, InitLeftFootAbsPos,
                        InitRightFootAbsPos);

  deque<RelativeFootPosition> RelativeFootPositions;
  m_ZMPVRQP->SetCurrentTime(m_InternalClock);

  m_ZMPVRQP->InitOnLine(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions, InitLeftFootAbsPos,
                        InitRightFootAbsPos, RelativeFootPositions, lStartingCOMState, lStartingZMPPosition);

  m_GlobalStrategyManager->Setup(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
...
void PatternGeneratorInterfacePrivate::initOnlineNaveau() {
  ...
  m_ZMPVRSQP->InitOnLine(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions, InitLeftFootAbsPos,
                         InitRightFootAbsPos, RelativeFootPositions, lStartingCOMState, lStartingZMPPosition);
#endif
  m_GlobalStrategyManager->Setup(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
...
void PatternGeneratorInterfacePrivate::EvaluateStartingState(COMState &lStartingCOMState,
  ...
  m_GlobalStrategyManager->EvaluateStartingState(lBodyInit, lStartingCOMState, lStartingZMPPosition,
...
void PatternGeneratorInterfacePrivate::AutomaticallyAddFirstStep(deque<RelativeFootPosition> &lRelativeFootPositions,
  ...
  ODEBUG("InitLeftFoot:" << InitLeftFootAbsPos.x << " " << InitLeftFootAbsPos.y << " " << InitLeftFootAbsPos.theta);

  ODEBUG("InitRightFoot:" << InitRightFootAbsPos.x << " " << InitRightFootAbsPos.y << " "
...
void PatternGeneratorInterfacePrivate::CommonInitializationOfWalking(
    COMState &lStartingCOMState, Eigen::Vector3d &lStartingZMPPosition, Eigen::VectorXd &BodyAnglesIni,
    FootAbsolutePosition &InitLeftFootAbsPos, FootAbsolutePosition &InitRightFootAbsPos,
    deque<RelativeFootPosition> &lRelativeFootPositions, vector<double> &lCurrentJointValues,
  ...
  for (unsigned int i = 0; i < lRelativeFootPositions.size(); i++) {
    ODEBUG(lRelativeFootPositions[i].sx << " " << lRelativeFootPositions[i].sy << " " << lRelativeFootPositions[i].sz
                                        << " " << lRelativeFootPositions[i].theta);
  }

  // Initialize consequently the ComAndFoot Realization object.
  Eigen::Matrix<double, 6, 1> lStartingWaistPose;
  m_GlobalStrategyManager->EvaluateStartingState(BodyAnglesIni, lStartingCOMState, lStartingZMPPosition,
                                                 lStartingWaistPose, InitLeftFootAbsPos, InitRightFootAbsPos);

  // Add the first step automatically when the corresponding
  // option is set on.
  if (m_AutoFirstStep) {
    AutomaticallyAddFirstStep(lRelativeFootPositions, InitLeftFootAbsPos, InitRightFootAbsPos, lStartingCOMState);
    if (!ClearStepStackHandler) {
      ...
      ODEBUG("Push a position in stack of steps:"
             << lRelativeFootPositions[0].sx << " " << lRelativeFootPositions[0].sy << " "
             << lRelativeFootPositions[0].sz << " " << lRelativeFootPositions[0].theta);
    }
  }

  ODEBUG("StartingCOMState: " << lStartingCOMState.x[0] << " " << lStartingCOMState.y[0] << " "
  ...
  if (0) {
    ...
    if (aof.is_open()) {
      for (unsigned int i = 0; i < lRelativeFootPositions.size(); i++) {
        aof << lRelativeFootPositions[i].sx << " " << lRelativeFootPositions[i].sy << " "
...
void PatternGeneratorInterfacePrivate::StartOnLineStepSequencing() {
  ...
  CommonInitializationOfWalking(lStartingCOMState, lStartingZMPPosition, BodyAnglesIni, InitLeftFootAbsPos,
                                InitRightFootAbsPos, lRelativeFootPositions, lCurrentJointValues, false);

  if (m_ZMPInitialPointSet) {
    for (unsigned int i = 0; i < 3; i++) lStartingZMPPosition(i) = m_ZMPInitialPoint(i);
  }

  ODEBUG("StartOnLineStepSequencing - 3 " << lStartingCOMState.x[0] << " " << lStartingCOMState.y[0] << " "
                                          << lStartingCOMState.z[0] << " " << lRelativeFootPositions.size());
  ODEBUG("ZMPInitialPoint OnLine" << lStartingZMPPosition(0) << " " << lStartingZMPPosition(1) << " "
  ...
  if (m_AlgorithmforZMPCOM == ZMPCOM_KAJITA_2003) {
    ...
    NbOfStepsToRemoveFromTheStack =
        m_ZMPD->InitOnLine(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions, InitLeftFootAbsPos,
                           InitRightFootAbsPos, lRelativeFootPositions, lStartingCOMState, lStartingZMPPosition);
    ...
    NbOfStepsToRemoveFromTheStack =
        m_ZMPM->InitOnLine(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions, InitLeftFootAbsPos,
                           InitRightFootAbsPos, lRelativeFootPositions, lStartingCOMState, lStartingZMPPosition);
  ...
  for (std::size_t i = 0; i < NbOfStepsToRemoveFromTheStack - 1; i++) m_StepStackHandler->RemoveFirstStepInTheStack();
  ...
  ODEBUG("StartOnLineStepSequencing - 4 " << m_ZMPPositions.size() << " " << m_LeftFootPositions.size() << " "
                                          << m_RightFootPositions.size() << " ");
}

void PatternGeneratorInterfacePrivate::StopOnLineStepSequencing() { m_StepStackHandler->StopOnLineStep(); }
...
void PatternGeneratorInterfacePrivate::FinishAndRealizeStepSequence() {
  ...
  deque<RelativeFootPosition> lRelativeFootPositions;
  CommonInitializationOfWalking(lStartingCOMState, lStartingZMPPosition, BodyAnglesIni, InitLeftFootAbsPos,
                                InitRightFootAbsPos, lRelativeFootPositions, lCurrentJointValues, true);

  ODEBUG("lStartingCOMState: " << lStartingCOMState.x[0] << " " << lStartingCOMState.y[0] << " "
                               << lStartingCOMState.z[0]);
  ...
  ODEBUG("ZMPInitialPoint" << lStartingZMPPosition(0) << " " << lStartingZMPPosition(1) << " "
                           << lStartingZMPPosition(2));

  ODEBUG("COMBuffer: " << m_COMBuffer.size());

  // Create the ZMP reference.
  CreateZMPReferences(lRelativeFootPositions, lStartingCOMState, lStartingZMPPosition, InitLeftFootAbsPos,
                      InitRightFootAbsPos);
  ...
  if (0) {
    m_ZMPD->DumpDataFiles("/tmp/ZMPSetup.dat", "/tmp/LeftFootAbsolutePosSetup.dat", m_ZMPPositions,
                          m_LeftFootPositions);

    m_ZMPD->DumpDataFiles("/tmp/ZMPSetup.dat", "/tmp/RightFootAbsolutePosSetup.dat", m_ZMPPositions,
                          m_RightFootPositions);
  }
...
void PatternGeneratorInterfacePrivate::m_ReadFileFromKineoWorks(istringstream &strm) {
  ...
  ODEBUG6(
      ...
      "istringstream &strm)",
      "DebugGMFKW.dat");
  if (m_GMFKW->ReadPartialModel(aPartialModel) < 0) cerr << "Error while reading partial model " << endl;

  if (m_GMFKW->ReadKineoWorksPath(aKWPath) < 0) cerr << "Error while reading the path " << endl;
  ODEBUG6(
...
void PatternGeneratorInterfacePrivate::CallMethod(string &aCmd, istringstream &strm) {
...
bool PatternGeneratorInterfacePrivate::RunOneStepOfTheControlLoop(Eigen::VectorXd &CurrentConfiguration,
  ...
  m_Running = RunOneStepOfTheControlLoop(CurrentConfiguration, CurrentVelocity, CurrentAcceleration, ZMPTarget,
...
bool PatternGeneratorInterfacePrivate::RunOneStepOfTheControlLoop(
    ...
    Eigen::VectorXd &ZMPTarget, COMPosition &finalCOMPosition, FootAbsolutePosition &LeftFootPosition,
    FootAbsolutePosition &RightFootPosition) {
  COMState aCOMState;
  m_Running = RunOneStepOfTheControlLoop(CurrentConfiguration, CurrentVelocity, CurrentAcceleration, ZMPTarget,
...
bool PatternGeneratorInterfacePrivate::RunOneStepOfTheControlLoop(Eigen::VectorXd &CurrentConfiguration,
                                                                  Eigen::VectorXd &CurrentVelocity,
                                                                  Eigen::VectorXd &CurrentAcceleration,
                                                                  Eigen::VectorXd &ZMPTarget, COMState &finalCOMState,
  ...
  if (m_StepStackHandler->IsOnLineSteppingOn()) {
    ...
    if (m_AlgorithmforZMPCOM == ZMPCOM_WIEBER_2006) {
      ...
      m_ZMPD->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
    } else if (m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) {
      ODEBUG("InternalClock:" << m_InternalClock << " SamplingPeriod: " << m_SamplingPeriod);

      m_ZMPM->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
    }
  } else
  /* Check if we are not in an ending phase generated on-line */
  {
    if ((m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) && (m_ZMPM->GetOnLineMode())) {
      m_ZMPM->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
    }
  }

  if (m_AlgorithmforZMPCOM == ZMPCOM_HERDT_2010) {
    m_ZMPVRQP->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
  ...
  if (m_AlgorithmforZMPCOM == ZMPCOM_NAVEAU_2015) {
    ...
    m_ZMPVRSQP->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
    m_Running = m_ZMPVRSQP->Running();

    m_ZMPVRSQP->UpdateCurrentPos(m_ZMPPositions[0], m_COMBuffer[0], m_LeftFootPositions[0], m_RightFootPositions[0]);
  }
#endif

  m_GlobalStrategyManager->OneGlobalStepOfControl(LeftFootPosition, RightFootPosition, ZMPTarget, finalCOMState,
                                                  CurrentConfiguration, CurrentVelocity, CurrentAcceleration);

  ODEBUG("finalCOMState: " << finalCOMState.x[0] << " " << finalCOMState.x[1] << " " << finalCOMState.x[2] << " "
                           << finalCOMState.y[0] << " " << finalCOMState.y[1] << " " << finalCOMState.y[2] << " "
                           << finalCOMState.z[0] << " " << finalCOMState.z[1] << " " << finalCOMState.z[2] << " "
                           << finalCOMState.yaw[0] << " " << finalCOMState.yaw[1] << " " << finalCOMState.yaw[2] << " "
                           << finalCOMState.pitch[0] << " " << finalCOMState.pitch[1] << " " << finalCOMState.pitch[2]
                           << " " << finalCOMState.roll[0] << " " << finalCOMState.roll[1] << " "
  ...
  ODEBUG4("CurrentWaistState: " << m_CurrentWaistState.x[0] << " " << m_CurrentWaistState.y[0] << " "
                                << m_CurrentWaistState.z[0] << " " << m_CurrentWaistState.roll[0] << " "
                                << m_CurrentWaistState.pitch[0] << " " << m_CurrentWaistState.yaw[0],
          "DebugDataWaist.dat");
  ...
  if (m_GlobalStrategyManager->EndOfMotion() == GlobalStrategyManager::NEW_STEP_NEEDED) {
    ...
    if (m_StepStackHandler->IsOnLineSteppingOn()) {
      ...
      ODEBUG(" EnoughSteps: " << EnoughSteps << endl << " EndSequence:" << EndSequence << endl);

      if (!EndSequence) {
        // *** WARNING THIS IS THE TIME CONSUMING PART ***
        if (m_AlgorithmforZMPCOM == ZMPCOM_WIEBER_2006) {
        } else if (m_AlgorithmforZMPCOM == ZMPCOM_KAJITA_2003) {
          m_ZMPD->OnLineAddFoot(lRelativeFootPositions, m_ZMPPositions, m_COMBuffer, m_LeftFootPositions,
                                m_RightFootPositions, EndSequence);

        } else if (m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) {
          ODEBUG("Putting a new step SX: " << lRelativeFootPositions.sx << " SY: " << lRelativeFootPositions.sy);
          m_ZMPM->SetCurrentTime(m_InternalClock);
          m_ZMPM->OnLineAddFoot(lRelativeFootPositions, m_ZMPPositions, m_COMBuffer, m_LeftFootPositions,
                                m_RightFootPositions, EndSequence);
          ODEBUG("Left and Right foot positions queues: " << m_LeftFootPositions.size() << " "
                                                          << m_RightFootPositions.size());
        }
      } else if (EndSequence) {
        ODEBUG("End Sequence");
        if (m_AlgorithmforZMPCOM == ZMPCOM_WIEBER_2006) {
          ...
          m_ZMPD->EndPhaseOfTheWalking(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
        } else if (m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) {
          ODEBUG("Putting a new step SX: " << lRelativeFootPositions.sx << " SY: " << lRelativeFootPositions.sy);
          m_ZMPM->SetCurrentTime(m_InternalClock);
          m_ZMPM->EndPhaseOfTheWalking(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions);
...
void PatternGeneratorInterfacePrivate::SetCurrentJointValues(Eigen::VectorXd &lCurrentJointValues) {
  if ((std::size_t)lCurrentJointValues.size() != m_CurrentActuatedJointValues.size())
...
int PatternGeneratorInterfacePrivate::GetWalkMode() const { return m_StepStackHandler->GetWalkMode(); }
...
void PatternGeneratorInterfacePrivate::GetLegJointVelocity(Eigen::VectorXd &dqr, Eigen::VectorXd &dql) const {
...
void PatternGeneratorInterfacePrivate::getWaistVelocity(double &dx, double &dy, double &omega) const {
  dx = m_AbsLinearVelocity(0);
  dy = m_AbsLinearVelocity(1);
  omega = m_AbsAngularVelocity(2);
}

void PatternGeneratorInterfacePrivate::setVelocityReference(double x, double y, double yaw) {
...
void PatternGeneratorInterfacePrivate::setCoMPerturbationForce(double x, double y) {
...
int PatternGeneratorInterfacePrivate::ChangeOnLineStep(double time, FootAbsolutePosition &aFootAbsolutePosition,
                                                       double &newtime) {
  /* Compute the index of the interval which will be modified. */
  if (m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) {
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPM->OnLineFootChange(m_InternalClock + time, aFootAbsolutePosition, m_ZMPPositions, m_COMBuffer,
                             m_LeftFootPositions, m_RightFootPositions, m_StepStackHandler);
    vector<double> lDj;
...
int PatternGeneratorInterfacePrivate::CreateZMPReferences(deque<RelativeFootPosition> &lRelativeFootPositions,
  ...
  if (m_AlgorithmforZMPCOM == ZMPCOM_WIEBER_2006) {
    ...
    m_COMBuffer.clear();
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPQP->GetZMPDiscretization(m_ZMPPositions, m_COMBuffer, lRelativeFootPositions, m_LeftFootPositions,
                                  m_RightFootPositions, m_Xmax, lStartingCOMState, lStartingZMPPosition,
  ...
  //   }
    ...
    m_COMBuffer.clear();
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPVRQP->GetZMPDiscretization(m_ZMPPositions, m_COMBuffer, lRelativeFootPositions, m_LeftFootPositions,
                                    m_RightFootPositions, m_Xmax, lStartingCOMState, lStartingZMPPosition,
                                    InitLeftFootAbsPos, InitRightFootAbsPos);
  }
#if USE_QUADPROG == 1
    ...
    m_COMBuffer.clear();
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPVRSQP->GetZMPDiscretization(m_ZMPPositions, m_COMBuffer, lRelativeFootPositions, m_LeftFootPositions,
                                     m_RightFootPositions, m_Xmax, lStartingCOMState, lStartingZMPPosition,
                                     InitLeftFootAbsPos, InitRightFootAbsPos);
  }
#endif
    ...
    m_ZMPD->GetZMPDiscretization(m_ZMPPositions, m_COMBuffer, lRelativeFootPositions, m_LeftFootPositions,
                                 m_RightFootPositions, m_Xmax, lStartingCOMState, lStartingZMPPosition,
                                 InitLeftFootAbsPos, InitRightFootAbsPos);
    ...
    m_COMBuffer.clear();
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPM->GetZMPDiscretization(m_ZMPPositions, m_COMBuffer, lRelativeFootPositions, m_LeftFootPositions,
                                 m_RightFootPositions, m_Xmax, lStartingCOMState, lStartingZMPPosition,
                                 InitLeftFootAbsPos, InitRightFootAbsPos);
