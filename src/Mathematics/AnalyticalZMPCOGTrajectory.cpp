...
void AnalyticalZMPCOGTrajectory::SetNumberOfIntervals(unsigned int lNbOfIntervals) {
  ...
  for (unsigned int i = 0; i < m_ListOfCOGPolynomials.size(); i++) m_ListOfCOGPolynomials[i] = 0;

  m_ListOfZMPPolynomials.resize(m_NbOfIntervals);
  for (unsigned int i = 0; i < m_ListOfZMPPolynomials.size(); i++) m_ListOfZMPPolynomials[i] = 0;
}
...
bool AnalyticalZMPCOGTrajectory::ComputeCOM(double t, double &r) {
  ...
  for (unsigned int j = 0; j < m_DeltaTj.size(); j++) {
    ...
    if (((t + m_Sensitivity) >= reftime) && (t <= reftime + m_DeltaTj[j] + m_Sensitivity)) {
      ...
      ODEBUG(" coefficients hyperbolique : " << r << " m_omegaj[" << j << "]=" << m_omegaj[j] << " deltaj " << deltaj
...
bool AnalyticalZMPCOGTrajectory::ComputeCOMSpeed(double t, double &r) {
  ...
  for (unsigned int j = 0; j < m_DeltaTj.size(); j++) {
    ...
    if (((t + m_Sensitivity) >= reftime) && (t <= reftime + m_DeltaTj[j] + m_Sensitivity)) {
      ...
      r = m_omegaj[j] * sinh(m_omegaj[j] * deltaj) * m_V[j] + m_omegaj[j] * cosh(m_omegaj[j] * deltaj) * m_W[j];
      ODEBUG(" coefficients hyperbolique : " << r << " m_omegaj[j]=" << m_omegaj[j] << " deltaj " << deltaj
                                             << " m_V[j]:" << m_V[j] << " m_W[j]:" << m_W[j]);
      if (m_ListOfCOGPolynomials[j] != 0) {
        double add2r;
...
void AnalyticalZMPCOGTrajectory::SetCoGHyperbolicCoefficients(vector<double> &lV, vector<double> &lW) {
...
void AnalyticalZMPCOGTrajectory::SetPolynomialDegrees(vector<unsigned int> &lPolynomialDegree) {
  if ((int)lPolynomialDegree.size() == m_NbOfIntervals) m_PolynomialDegree = lPolynomialDegree;
...
void AnalyticalZMPCOGTrajectory::GetHyperbolicCoefficients(vector<double> &lV, vector<double> &lW) const {
  lV = m_V;
  lW = m_W;
}

void AnalyticalZMPCOGTrajectory::GetStartingPointAndHeightVariation(vector<double> &lTj, vector<double> &lomegaj) {
  lTj = m_DeltaTj;
  lomegaj = m_omegaj;
}
...
void AnalyticalZMPCOGTrajectory::TransfertOneIntervalCoefficientsFromCOGTrajectoryToZMPOne(unsigned int intervalindex,
  ...
  if ((m_ListOfCOGPolynomials[intervalindex] != 0) && (m_ListOfZMPPolynomials[intervalindex] != 0)) {
...
void AnalyticalZMPCOGTrajectory::Building3rdOrderPolynomial(unsigned int anIntervalj, double pjTjm1, double pjTj) {
...
double AnalyticalZMPCOGTrajectory::FluctuationMaximal() {
  ...
  Tmax = m_AbsoluteTimeReference -
         2 * CoefsForZMP[2] * m_DeltaTj[0] / (2.0 * CoefsForZMP[2] + 3.0 * CoefsForZMP[3] * m_DeltaTj[0]);
