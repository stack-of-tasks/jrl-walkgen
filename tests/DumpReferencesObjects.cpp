...
void DumpReferencesObjects::setAnklePositions(Eigen::Vector3d &AnklePositionRight,
                                              Eigen::Vector3d &AnklePositionLeft) {
  m_AnklePositionRight = AnklePositionRight;
  m_AnklePositionLeft = AnklePositionLeft;
}
void DumpReferencesObjects::prepareFile(ofstream &aof, string &prefix, struct OneStep &anOneStep) {
  string aFileName;
  aFileName = prefix;
  aFileName += ".dat";
...
void DumpReferencesObjects::fillFileWithSubsamplingAndClose(FillingFileArgs_t &aSetOfFillingFileArgs,
                                                            std::vector<double> &next, std::vector<double> &prev) {
  for (double subsampling = 1.0; subsampling <= aSetOfFillingFileArgs.nb_subsampling; subsampling += 1.0) {
    if (m_TimeOption) {
      aSetOfFillingFileArgs.aof << filterprecision(
                                       ((double)aSetOfFillingFileArgs.anOneStep.m_NbOfIt) * aSetOfFillingFileArgs.dt +
                                       subsampling * aSetOfFillingFileArgs.dt / aSetOfFillingFileArgs.nb_subsampling)
                                << " ";  // 1
    }
    for (unsigned int i = 0; i < next.size(); i++) {
      double intermediate = prev[i] + (next[i] - prev[i]) * subsampling / aSetOfFillingFileArgs.nb_subsampling;
...
void DumpReferencesObjects::fillInTests(std::string &aTestName, struct OneStep &anOneStep,
...
void DumpReferencesObjects::fillInTestsFormat1(std::string &aTestName, struct OneStep &anOneStep,
                                               Eigen::VectorXd &aCurrentConfiguration)

{
  ...
  double localZMPx = anOneStep.m_ZMPTarget(0) * cos(aCurrentConfiguration(5)) -
                     anOneStep.m_ZMPTarget(1) * sin(aCurrentConfiguration(5)) + aCurrentConfiguration(0);
  double localZMPy = anOneStep.m_ZMPTarget(0) * sin(aCurrentConfiguration(5)) +
                     anOneStep.m_ZMPTarget(1) * cos(aCurrentConfiguration(5)) + aCurrentConfiguration(1);
  ...
  prepareFile(aof, prefix, anOneStep);
  vec_db[0] = filterprecision(anOneStep.m_LeftFootPosition.x + m_AnklePositionLeft[0]);  // 17
  vec_db[1] = filterprecision(anOneStep.m_LeftFootPosition.y + m_AnklePositionLeft[1]);  // 18
  vec_db[2] = filterprecision(anOneStep.m_LeftFootPosition.z + m_AnklePositionLeft[2]);  // 19
  ...
  prepareFile(aof, prefix, anOneStep);
  vec_db[0] = filterprecision(anOneStep.m_RightFootPosition.x + m_AnklePositionRight[0]);
  vec_db[1] = filterprecision(anOneStep.m_RightFootPosition.y + m_AnklePositionRight[1]);
  vec_db[2] = filterprecision(anOneStep.m_RightFootPosition.z + m_AnklePositionRight[2]);
...
void DumpReferencesObjects::fillInTestsFormat2(std::string &aTestName, struct OneStep &anOneStep,
                                               Eigen::VectorXd &aCurrentConfiguration)

{
  ...
  TestSuite::FillingFileArgs_t aSetOfFillingFileArgs(aof, dt, nb_sampling, anOneStep);
  ...
  prepareFile(aof, prefix, anOneStep);
  vec_db.resize(18);
  vec_db[0] = filterprecision(anOneStep.m_LeftFootPosition.x + m_AnklePositionLeft[0]);
  vec_db[1] = filterprecision(anOneStep.m_LeftFootPosition.y + m_AnklePositionLeft[1]);
  vec_db[2] = filterprecision(anOneStep.m_LeftFootPosition.z + m_AnklePositionLeft[2]);
  ...
  if (m_prevTorquesF2.size() != (std::vector<double>::size_type)aTauVec.size()) m_prevTorquesF2.resize(aTauVec.size());

  for (Eigen::VectorXd::Index i = 0; i < aTauVec.size(); i++) vec_db[i] = aTauVec[i];
